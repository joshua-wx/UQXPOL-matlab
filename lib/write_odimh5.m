...
%create h5 ffn
scan_date = datestr(data_struct.header.file_datetime,'yyyymmdd_HHMM');
...
%write datasets
write_hdf_v2(h5_ffn,data_struct,endtime_offset);
...
function write_hdf_v2(h5_fn,data_struct,endtime_offset)
...
end_datetime = addtodate(data_struct.header.file_datetime,endtime_offset,'minute'); %manual offset in minutes
H5Acreatestring(g_id, 'enddate', datestr(end_datetime,'yyyymmdd'));
H5Acreatestring(g_id, 'endtime', datestr(end_datetime,'HHMMSS'));
...
if strcmp(scan_type,'scn') || strcmp(scan_type,'sppi') %scn and sppi where group
    ...
    rstart  = data_struct.data.scan_rng(1);
    ...
    range    =  data_struct.data.scan_rng(end);
...
%RAIN
create_data(1,sort_ind,group_id,data_struct.data.scan_rain,'RATE',0,0);
%DBZH
create_data(2,sort_ind,group_id,data_struct.data.scan_zhh,'DBZH',0,0);
%VRAD
create_data(3,sort_ind,group_id,data_struct.data.scan_vel,'VRADH',0,0);
%ZRD
create_data(4,sort_ind,group_id,data_struct.data.scan_zdr,'ZDR',0,0);
%KDP
create_data(5,sort_ind,group_id,data_struct.data.scan_kdp,'KDP',0,0);
%PHIDP
create_data(6,sort_ind,group_id,data_struct.data.scan_phidp,'PHIDP',0,0);
%RHOHV
create_data(7,sort_ind,group_id,data_struct.data.scan_rhohv,'RHOHV',0,0);
%WRAD
create_data(8,sort_ind,group_id,data_struct.data.scan_specwidth,'WRADH',0,0);
%QC
...
function create_data(index,sort_ind,group_id,data,quantity,nodata,undetect)
...
if max(data(:))>int16(inf)
    display('data value larger than int16')
    return
end
...
H5Acreatedouble(g_id, 'gain', 1.0); %float
H5Acreatedouble(g_id, 'offset', 0.0); %float
H5Acreatedouble(g_id, 'nodata', nodata);
H5Acreatedouble(g_id, 'undetect', undetect);
...
dataset_id = H5D.create(data_id, 'data','H5T_STD_I16LE',dataspace_id, 'H5P_DEFAULT', plist, 'H5P_DEFAULT');

%write data variable
H5D.write(dataset_id, 'H5T_STD_I16LE', 'H5S_ALL', 'H5S_ALL', 'H5P_DEFAULT', data_set);
